...
export class ToolRegistry {
  ...
  async refresh(allowedServers = null, signal) {
    ...
    for (const [server, client] of this.clients) {
      ...
      try {
        ...
        console.error(JSON.stringify({ event: 'mcp.list_tools.failed', server, error: error.message }));
      }
    }
    return this.tools;
  }
  ...
  async execute(name, args, userText, signal) {
    ...
    try {
      const result = await this.clients.get(tool.server).callTool(tool.remoteName, args, signal);
