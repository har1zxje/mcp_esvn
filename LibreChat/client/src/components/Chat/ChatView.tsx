...
import ContextPanel, { type ContextPanelSection } from './ContextPanel';
...
function ChatView({ index = 0, project }: { index?: number; project?: TChatProject }) {
  const [contextPanelCollapsed, setContextPanelCollapsed] = useState(() => {
    try {
      return window.localStorage.getItem('vfiContextPanelCollapsed') === 'true';
    } catch {
      return false;
    }
  });
  const [contextPanelSection, setContextPanelSection] = useState<ContextPanelSection>('quick');

  useEffect(() => {
    window.localStorage.setItem('vfiContextPanelCollapsed', String(contextPanelCollapsed));
                ...
                <TraceSurface conversationId={conversationId}>
                  ...
                  <div className="flex min-h-0 flex-1">
                    ...
                    <ContextPanel
                      ...
                      onToggle={() => setContextPanelCollapsed((value) => !value)}
                      section={contextPanelSection}
                      onSectionChange={setContextPanelSection}
                      parentConversationId={parentConversationId}
                      readOnly={isSubagentThreadReadOnly}
                    />
                  </div>
