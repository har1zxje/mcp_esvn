...
class TestPagination(unittest.TestCase):
    ...
    def test_list_tables_pagination(self):
        ...
        result = json.loads(list_tables(self.test_db, page_size=3))
        ...
        result2 = json.loads(list_tables(self.test_db, page_token=page_token, page_size=3))
        ...
        result3 = json.loads(list_tables(self.test_db, page_token=page_token, page_size=3))
        self.assertEqual(len(result3["tables"]), 3)
        self.assertIsNotNone(result3["next_page_token"])

        page_token = result3["next_page_token"]
        result4 = json.loads(list_tables(self.test_db, page_token=page_token, page_size=3))
    ...
    def test_invalid_page_token(self):
        ...
        result = json.loads(list_tables(self.test_db, page_token="invalid_token", page_size=3))
        self.assertIsInstance(result, dict)
        self.assertIn("tables", result)
        self.assertIn("next_page_token", result)
        self.assertEqual(len(result["tables"]), 3)
    ...
    def test_token_for_different_database(self):
        ...
        try:
            ...
            result2 = json.loads(list_tables(test_db2, page_token=page_token, page_size=3))
    ...
    def test_different_page_sizes(self):
        ...
        result = json.loads(list_tables(self.test_db, page_size=20))
        self.assertEqual(len(result["tables"]), 10)
        self.assertIsNone(result["next_page_token"])

        result = json.loads(list_tables(self.test_db, page_size=5))
        self.assertEqual(len(result["tables"]), 5)
        self.assertIsNotNone(result["next_page_token"])

        page_token = result["next_page_token"]
        result2 = json.loads(list_tables(self.test_db, page_token=page_token, page_size=5))
    ...
    def test_filters_with_pagination(self):
        ...
        result = json.loads(list_tables(self.test_db, like="test_table_%", page_size=5))
        self.assertEqual(len(result["tables"]), 5)
        self.assertIsNotNone(result["next_page_token"])

        result2 = json.loads(list_tables(
            self.test_db, like="test_table_%", page_token=result["next_page_token"], page_size=5
        ))
        self.assertEqual(len(result2["tables"]), 5)
        self.assertIsNone(result2["next_page_token"])

        result3 = json.loads(list_tables(self.test_db, not_like="test_table_1%", page_size=10))
    ...
    def test_metadata_trimming(self):
        ...
        result_with_columns = json.loads(
            list_tables(self.test_db, page_size=3, include_detailed_columns=True)
        )
        ...
        result_without_columns = json.loads(list_tables(
    ...
    def test_metadata_trimming_with_pagination(self):
        ...
        result1 = json.loads(
            list_tables(self.test_db, page_size=3, include_detailed_columns=False)
        )
