...
import { ProjectChatBaseURL, QueryKeys, request, dataService } from 'librechat-data-provider';
...
export const fetchStreamStatus = async (conversationId: string): Promise<StreamStatusResponse> => {
  return request.get<StreamStatusResponse>(
    withGenerationProtocolQuery(`${ProjectChatBaseURL}/status/${conversationId}`),
