...
export default function Landing() {
  ...
  // The VFI landing is a workspace greeting; technical agent/model names stay
  // available in the existing header selector and are not the hero copy.
  const name = '';
  ...
  const greetingText = isTemporary
    ...
    : (resolvedWelcome ?? (user?.name ? `Xin chào ${user.name}!` : 'Xin chào!'));
  const workspaceDescription = isTemporary
    ? description
    : description || 'Tôi có thể hỗ trợ gì cho bạn hôm nay?';

  return (
    <div
      className={`flex min-h-0 flex-1 transform-gpu flex-col items-center justify-center pb-16 pt-16 transition-all duration-200 sm:pt-20 max-h-full ${getDynamicMargin}`}
    >
        ...
        {workspaceDescription &&
          (descriptionIsHTML ? (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: sanitizeDescription(workspaceDescription) }}
            />
          ) : (
            <div
