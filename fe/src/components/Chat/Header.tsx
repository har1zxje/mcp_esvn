...
import { PROJECT_NAME, PROJECT_USER_MODE } from '~/config/project';
...
function Header({
  ...
  return (
    <div className="chat-header absolute top-0 z-10 flex h-[52px] w-full items-center gap-2 bg-gradient-to-b from-presentation via-presentation/70 to-transparent p-2 font-semibold text-text-primary md:from-presentation/80 md:via-presentation/50 2xl:from-presentation/0 2xl:via-transparent">
      ...
      >
        ...
        {PROJECT_USER_MODE ? (
          <>
            <span className="hidden truncate px-2 text-sm font-medium text-text-secondary sm:inline">
              {PROJECT_NAME}
            </span>
            {!readOnly && <ProjectModelSelector />}
          </>
        ) : (
          !readOnly && <ModelSelector startupConfig={startupConfig} />
        )}
        {!PROJECT_USER_MODE && !readOnly && interfaceConfig.presets === true && interfaceConfig.modelSelect === true && (
          <PresetsMenu />
        )}
