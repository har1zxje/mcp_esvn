...
import * as Menu from '@ariakit/react';
...
function HelpSubmenu({
  ...
  return (
    <Menu.MenuProvider store={menu} placement="right-start">
      <Menu.MenuItem
        hideOnClick={false}
        render={
          <Menu.MenuButton
            store={menu}
            className="select-item flex w-full cursor-pointer items-center gap-2 text-sm"
          />
        }
      >
...
function AccountSettings({ collapsed = false }: { collapsed?: boolean }) {
  ...
  return (
    <Menu.MenuProvider store={menu} placement={collapsed ? 'right-end' : undefined}>
      <Menu.MenuButton
        store={menu}
