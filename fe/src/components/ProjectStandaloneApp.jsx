...
async function readChatResponse(response, onStreamText) {
  ...
  while (true) {
    ...
    for (const event of events) {
      ...
      try { const payload = JSON.parse(data); if (payload.usage) window.dispatchEvent(new CustomEvent('project-context-usage', { detail: payload.usage })); finalPayload = payload.conversation ? payload : finalPayload; const chunk = payload.text || payload.delta || payload.content || ''; if (chunk) { text += chunk; onStreamText(text); } } catch { /* tolerate keepalive events */ }
    }
  }
...
export default function ProjectStandaloneApp() {
  ...
  const [error, setError] = useState(''); const controllerRef = useRef(null); const leftResizeRef = useRef(null);
  ...
  const newConversation = () => {
    ...
    setMessages([]); setConversationId(null); setSelectedServers([]); saveMcpSelection('new', []); setError(''); setIsSending(false);
    navigate('/chat/new');
  };
  ...
  const send = async (event) => {
    event.preventDefault(); const message = value.trim(); if (!message || isSending) return; setValue(''); setError(''); setMessages((current) => [...current.filter((item) => !item.streaming), { role: 'user', text: message, createdAt: Date.now() }]); setIsSending(true); const controller = new AbortController(); controllerRef.current = controller;
    ...
    catch (sendError) { if (sendError.name !== 'AbortError') { console.error('[project-chat] request failed', { status: sendError.status, code: sendError.code, requestId: sendError.requestId, message: sendError.message }); setError(sendError.message); } } finally { controllerRef.current = null; setIsSending(false); }
  };
  ...
  return <div className={`project-app project-theme-${theme}${leftSidebarCollapsed ? ' left-sidebar-collapsed' : ''}${rightSidebarCollapsed ? ' right-sidebar-collapsed' : ''}`} style={{ '--project-sidebar-width': `${leftSidebarWidth}px` }}>
    ...
    <main className="project-main"><ChatHeader models={models} modelId={modelId} onModelChange={setModelId} onNewConversation={newConversation} /><section className="project-messages" aria-live="polite">{groupedMessages.length === 0 ? <div className="project-empty"><div className="project-empty-logo">✦</div><h2>Xin chào Hải Đào!</h2><p>Tôi có thể hỗ trợ gì cho bạn hôm nay?</p></div> : groupedMessages.map(({ message, toolCalls }, index) => <ProjectMessage key={`${message.role}-${message.createdAt || index}`} message={message} toolCalls={toolCalls} modelLabel={activeModel?.label} />)}{isSending && <div className="project-status"><span className="project-spinner" /> Gemini đang suy nghĩ và gọi MCP…</div>}</section><ChatComposer value={value} setValue={setValue} onSend={send} isSending={isSending} servers={servers} selectedServers={selectedServers} onToggleServer={toggleServer} contextUsage={contextUsage} placeholder={`Message ${activeModel?.label || 'Gemini'}`} /><footer className="project-disclaimer">{error || `${connectedCount}/${servers.length} MCP server đã kết nối · AI có thể mắc lỗi, hãy kiểm tra thông tin quan trọng.`}</footer></main>
