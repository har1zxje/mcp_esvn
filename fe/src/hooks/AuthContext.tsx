...
import { PROJECT_GUEST_MODE } from '~/config/project';
...
const AuthContextProvider = ({
  ...
  // This project uses a server-owned guest session. The browser must not log in
  // to LibreChat; the BE owns the upstream connection instead.
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(PROJECT_GUEST_MODE);
  const [isAuthReady, setIsAuthReady] = useState<boolean>(
    PROJECT_GUEST_MODE || authConfig?.test === true,
  );
  ...
  const { data: userRole = null } = useGetRole(SystemRoles.USER, {
    enabled: !PROJECT_GUEST_MODE && !!(isAuthenticated && (user?.role ?? '')),
  });
  const { data: adminRole = null } = useGetRole(SystemRoles.ADMIN, {
    enabled: !PROJECT_GUEST_MODE && !!(isAuthenticated && user?.role === SystemRoles.ADMIN),
  });
  const { data: customRole = null } = useGetRole(isCustomRole ? userRoleName : '_', {
    enabled: !PROJECT_GUEST_MODE && isCustomRole,
  });
  ...
  const userQuery = useGetUserQuery({ enabled: !PROJECT_GUEST_MODE && !!(token ?? '') });
  ...
  const memoedValue = useMemo(
    () => ({
      ...
      setError,
      roles: {
        [SystemRoles.USER]: PROJECT_GUEST_MODE ? {} : userRole,
