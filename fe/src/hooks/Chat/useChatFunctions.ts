...
export default function useChatFunctions({
  ...
  const ask: TAskFunction = (
    ...
    // Project mode only exposes server-managed Agents. Never let a stale
    // LibreChat endpoint (for example openAI) bypass the project facade.
    const endpoint = PROJECT_USER_MODE ? EModelEndpoint.agents : conversation?.endpoint;
