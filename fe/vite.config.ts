...
// The project backend binds to 127.0.0.1 by default. Using `localhost` here
// can resolve to ::1 on Windows while the backend is IPv4-only, which makes
// Vite proxy requests hang even though both services are running.
const chatBackendHost = process.env.CHAT_BACKEND_HOST || '127.0.0.1';
const chatBackendURL = `http://${chatBackendHost}:${chatBackendPort}`;
...
export default defineConfig(({ command }) => ({
  ...
  server: {
    ...
    strictPort: false,
    proxy: {
      // The standalone frontend has one browser-facing API: the project BE.
      '/api/chat': {
        target: chatBackendURL,
        changeOrigin: true,
      },
      // All LibreChat-compatible UI data also goes through the project BE.
      // The browser never connects directly to LibreChat.
      '/api': {
        target: chatBackendURL,
        changeOrigin: true,
      },
    ...
    VitePWA({
      ...
      workbox: {
        ...
        globIgnores: [
          ...
          // These are loaded only on demand. Pre-caching them makes the first
          // PWA install download several megabytes that project mode does not
          // need to render the chat screen.
          'assets/mermaid.*.js',
  ...
  resolve: {
    // The local LibreChat packages are linked from ../LibreChat. Force every
    // linked component and the app itself to use one React runtime instance.
